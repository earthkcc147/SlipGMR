...
def signup():
    ...
    while True:
        # ตั้งเวลาจับเวลา 30 วินาที
        countdown = 30

        while countdown > 0:
            print(f"🔑 กรุณากรอก UUID ที่ได้รับจาก Discord (หรือกด 0 เพื่อยกเลิก) [เหลือเวลา: {countdown} วินาที]: ", end="\r")
            start_time = time.time()  # บันทึกเวลาเริ่มต้น
            input_uuid = input().strip()  # รับค่าจากผู้ใช้
            elapsed_time = time.time() - start_time  # คำนวณเวลาที่ผ่านไป

            # หากผู้ใช้กด 0 เพื่อยกเลิก
            if input_uuid == "0":
                print("\n⏪ ยกเลิกการสมัครสมาชิก กำลังกลับไปยังเมนูหลัก...")
                start_menu()  # กลับไปยังเมนูหลัก
                return  # ออกจากฟังก์ชัน

            # ตรวจสอบว่า UUID ที่กรอกตรงกับที่ส่งไป
            if input_uuid == user_uuid:
                print("\n✅ UUID ถูกต้อง! ดำเนินการสมัครสมาชิก...")
            ...
            # ลดเวลาจับเวลาตามเวลาที่ใช้
            countdown -= int(elapsed_time)

        # หากหมดเวลา 30 วินาที
