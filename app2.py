...
def countdown_timer(seconds, stop_event):
    """
    ฟังก์ชันสำหรับแสดงเวลาถอยหลัง
    """
    while seconds > 0 and not stop_event.is_set():
        print(f"\r⏳ เหลือเวลา: {seconds} วินาที", end="")
        time.sleep(1)
        seconds -= 1

    if not stop_event.is_set():
        print("\n⏳ เวลาหมด! ระบบยกเลิกการสมัครสมาชิก...")
...
def signup():
    ...
    stop_event = threading.Event()  # Event สำหรับหยุดตัวจับเวลา
    countdown_seconds = 30  # จำนวนวินาทีในการนับถอยหลัง

    # เริ่มตัวจับเวลาถอยหลังใน Thread แยก
    timer_thread = threading.Thread(target=countdown_timer, args=(countdown_seconds, stop_event))
    timer_thread.start()

    while True:
        input_uuid = input("\n🔑 กรุณากรอก UUID ที่ได้รับจาก Discord (หรือกด 0 เพื่อยกเลิก): ").strip()

        # หยุดตัวจับเวลาเมื่อได้รับอินพุต
        stop_event.set()
        timer_thread.join()  # รอให้ Thread ตัวจับเวลาหยุดทำงาน
        ...
        # หากต้องการเริ่มตัวจับเวลาใหม่ (กรณีผู้ใช้กรอก UUID ผิด)
        stop_event.clear()
        timer_thread = threading.Thread(target=countdown_timer, args=(countdown_seconds, stop_event))
        timer_thread.start()
