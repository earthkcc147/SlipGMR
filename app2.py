...
# ฟังก์ชันสำหรับตรวจสอบประเภทภาษาที่พบในข้อความ
# Function to check the type of language found in a text
def detect_language(text):
    ...
    if contains_numbers(text):
        detected_languages.append("Numbers")

    return detected_languages if detected_languages else ["ไม่พบตัวอักษรที่รู้จัก"]
...
def signup():
    ...
    while True:
        ...
        username_languages = detect_language(username)
        password_languages = detect_language(password)

        print(f"\n🔍 ตรวจพบในชื่อผู้ใช้: {', '.join(username_languages)}")
        print(f"🔍 ตรวจพบในรหัสผ่าน: {', '.join(password_languages)}")

        # ตรวจสอบหากมีภาษาไทย
        if "ภาษาไทย" in username_languages or "ภาษาไทย" in password_languages:
            print("❌ ห้ามใช้ภาษาไทย กรุณาใช้เฉพาะภาษาอังกฤษและตัวเลขเท่านั้น!")
            print("🔁 กรุณาลองใหม่!")
            continue

        # ตรวจสอบหากช่องใดช่องหนึ่งว่างเปล่า
        if not username or not password:
            print("❌ ข้อมูลไม่ครบถ้วน กรุณากรอกชื่อผู้ใช้และรหัสผ่านให้ครบ!")
            continue
