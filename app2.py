...
def print_centered(message, terminal_width):
...
def start_menu():
    ...
    while True:
        ...
        # คำนวณความยาวสูงสุดในเมนู เพื่อให้ทุกบรรทัดมีความยาวพอดีกัน
        max_length = max(wcswidth(item) for item in menu_items)

        # พิมพ์เมนูแต่ละบรรทัดโดยให้ตัวอักษรแรกเท่ากัน
        for item in menu_items:
            centered_item = print_centered(item, terminal_width)
            print(centered_item)
        ...
        if choice == "1":
            clear_console()
            print_centered("🔑 กำลังเข้าสู่ระบบ...", terminal_width)
            login()
        elif choice == "2":
            clear_console()
            print_centered("✍️ กำลังสมัครสมาชิก...", terminal_width)
            signup()
        elif choice == "3":
            clear_console()
            print_centered("🔄 กำลังอัปเดตโค้ดและรีสตาร์ท...", terminal_width)
            autoupdate_restart()  # เรียกฟังก์ชันอัปเดตและรีสตาร์ท
        elif choice == "00":
            clear_console()
            print_centered("👋 ออกจากโปรแกรม... สวัสดีครับ!", terminal_width)
            exit()
        else:
            print_centered("⚠️❌ ตัวเลือกไม่ถูกต้อง กรุณาลองใหม่.", terminal_width)
