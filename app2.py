...
def print_centered_boxed_messages(messages):
    padding = 2  # ช่องว่างซ้าย-ขวาอย่างน้อย
...
def print_boxed_menu(menu_items):
    ...
    total_width = max_length + 4  # กว้างทั้งหมด (รวมขอบซ้ายและขวา)
    ...
    for item in formatted_items:
        real_length = wcswidth(item)  # ความกว้างที่แท้จริงของข้อความ
        padding = total_width - real_length - 2  # คำนวณช่องว่างด้านขวา
...
def print_top_right_boxed_messages(messages):
    ...
    # คำนวณการจัดตำแหน่งให้กรอบอยู่ที่มุมขวาและเลื่อนขยับไปทางซ้าย
    padding_left = terminal_width - total_length - 4  # เลื่อนกรอบไปทางซ้ายเล็กน้อย
...
def display_username():
    if global_status["login_status"]:
        ...
        # แสดงชื่อผู้ใช้ในมุมขวาบน
        print_top_right(username_message)  # ส่งเป็นข้อความเดียว
    else:
        # ถ้าไม่ได้เข้าสู่ระบบ
        print_top_right("🚫 ยังไม่ได้เข้าสู่ระบบ.")  # แสดงข้อความนี้ถ้ายังไม่ได้ล็อกอิน


# ฟังก์ชันที่ปรับปรุงเพื่อแสดงชื่อผู้ใช้จาก global_status ในกรอบ
def display_username_box():
    if global_status["login_status"]:
        ...
        # ถ้าไม่ได้เข้าสู่ระบบ
        print_top_right_boxed_messages(["🚫 ยังไม่ได้เข้าสู่ระบบ."])  # แสดงข้อความนี้ถ้ายังไม่ได้ล็อกอิน
...
def signup():
    ...
    while True:
        print()
        # แสดงข้อมูลที่กรอกเพื่อให้ผู้ใช้ยืนยัน
        # print_boxed_message("🔎 กรุณาตรวจสอบข้อมูลที่กรอก:")
        # print(f"👤 ชื่อผู้ใช้: {username}")
        # print(f"🔒 รหัสผ่าน: {password}\n")
...
# ฟังก์ชันสำหรับเพิ่มผู้ใช้ใหม่ใน USERS ติดตั้ง pip install python-dotenv
...
def start_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            clear_console()
            print_centered("👋 ออกจากโปรแกรม... สวัสดีครับ!")
            exit()
        else:
            print_centered("⚠️❌ ตัวเลือกไม่ถูกต้อง กรุณาลองใหม่.")
...
def send_exit_message_to_discord(username):
    title = "🚪 ผู้ใช้ออกจากระบบ"  # กำหนด title ที่ต้องการ
    message = f"🚪 ผู้ใช้ {username} ออกจากโปรแกรม"
    sdmc2(title, message)  # ส่งข้อความออกไปที่ Discord
...
def select_bank():
    ...
    Bank = [
        ...
        "3. |  Bangkok 🏙️ (ธนาคารกรุงเทพ)",
        "4. |  TTB 🏛️ (ธนาคารทหารไทยธนชาต)",
        "5. |  กรุงศรี 🏦 (ธนาคารกรุงศรีอยุธยา)",
        "6. |  กรุงไทย 🏦 (ธนาคารกรุงไทย)",
        "7. |  TrueWallet 💳 (ทรูวอลเล็ท)",  # เพิ่ม TrueWallet
        "8. |  Other Bank 🎨 (ธนาคารอื่นๆ)",  # ตัวเลือกธนาคารอื่นๆ
        "00. |  ออกจากระบบ"  # เพิ่มตัวเลือกออกจากโปรแกรม
    ]

    # แสดงเมนูในกรอบ
    print_boxed_menu(Bank)
    print()
...
def select_background(bank_name):
    ...
    select_theme()
    print()
    print_centered(f"เลือกธีม {bank_name} ที่ต้องการ:")
    print()

    # กำหนดเมนูสำหรับธนาคารแต่ละประเภท
    if bank_name == "K-bank":
        menu_items = [
            "1. | K-bank ธรรมดา 🌇",
            "2. | K-bank คริสมาสต์ 🌆",
    ...
    choice = input("กรุณาเลือกหมายเลข (0-5) หรือ 00 เพื่อย้อนกลับ: ")
...
def select_logo(background_image):
    ...
    menu_items = [
        "1. | K-bank 🏦 (ธนาคารกสิกรไทย)",
        "2. | SCB 🏦 (ธนาคารไทยพาณิชย์)",
        "3. | Bangkok 🏦 (ธนาคารกรุงเทพ)",
        "4. | ออมสิน 🏦 (ธนาคารออมสิน)",
        "5. | กรุงศรี 🏦 (ธนาคารกรุงศรีอยุธยา)",
        "6. | กรุงไทย 🏦 (ธนาคารกรุงไทย)",
        "7. | UOB 🏦 (ธนาคารยูโอบี)",
        "8. | Citi 🏦 (ธนาคารซิตี้แบงก์)",
        "9. | TTB 🏦 (ธนาคารทหารไทยธนชาต)",
        "10. | Another Logo 🎨 (โลโก้ธนาคารอื่นๆ)",
        "00. | เริ่มใหม่ 🔄"
    ]

    # ใช้ print_boxed_menu สำหรับแสดงเมนู
    print_boxed_menu(menu_items)
