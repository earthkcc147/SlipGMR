...
def create_fake_receipt(name_user_id, name_me_id, phone_me_id, money_id):
    ...
    # กำหนดชื่อไฟล์ที่บันทึกตามรูปแบบ (ชื่อผู้โอน)_(ชื่อผู้รับ)_(จำนวนเงิน).png
    file_name = f"{name_user_id}_{name_me_id}_{money_id}.png"
    file_path = os.path.join(folder_path, file_name)

    # บันทึกภาพที่มีข้อความในโฟลเดอร์ "textnew"
    print("💾 กำลังบันทึกไฟล์... 📸")
    image.save(file_path)
    print(f"✅ สลีปปลอมสำเร็จ! บันทึกเป็น {file_path}")